...
```tikz
\draw[->] (0,0) -- (5.5,0);
\node[below] at (2.75,-0.5) {False Positive Rate};
\draw[->] (0,0) -- (0,5.5);
\node[left, rotate=90] at (-1,4) {True Positive Rate};
...
% Axis labels
\foreach \x/\label in {, 1/0.2, 2/0.4, 3/0.6, 4/0.8, 5/1}
...
\draw[thick, smooth] plot coordinates {
    (0,0) (0.5,2.5) (0.8,3.5) (1.2,4.05) (1.8,4.4) (2.5,4.6) (3.5,4.8) (5,5)
};

% Labels
\node at (3,1.5) {AUC};
\node[above right] at (1.2,3) {ROC Curve};
```
...
\draw[->] (-6,0) -- (6.5,0);
\node[below] at (0,-0.5) {x};
\draw[->] (0,-0.5) -- (0,5.5);
\node[left] at (-0.5,2.5) {f(x)};

% Axis labels
\node[above right] at (0,0) {0};
\node[above right] at (0,5) {1};

% Sigmoid curve using Bezier curves (real function broken during render)
\draw[thick] (-6,0.05)
  .. controls (-3,0.1) and (-2,0.5) .. (0,2.5)
  .. controls (2,4.5) and (3,4.9) .. (6,4.95);

% Key point
\fill (0,2.5) circle (1.5pt);
\node[below right] at (0,2.5) {0.5};

% Asymptotes
\draw[dashed] (-6,5) -- (6,5);

% Labels
\node at (0,6) {Logistic Function};
```
...
\draw[->] (0,-0.5) -- (0,5.5);
\node[left, rotate=90] at (-1,3) {H(p)};

% Axis labels - only 0, 0.5, 1
\foreach \x/\label in {2.5/0.5, 5/1}
    \node[below] at (\x,0) {\label};
\foreach \y/\label in {2.5/0.5, 5/1}
    \node[left] at (0,\y) {\label};

\node[below left] at (0,0) {0};

% True entropy function with smooth interpolation
\draw[thick, smooth, domain=0.01:4.99, samples=50]
    plot (\x, {
        5 * (
            -(\x/5) * (ln(\x/5) / ln(2)) -
            (1 - \x/5) * (ln(1 - \x/5) / ln(2))
...
% Manually connect to endpoints (limit is 0)
\draw[thick] (0,0) -- (0.05,{5 * (-0.01 * (ln(0.01) / ln(2)) - 0.99 * (ln(0.99) / ln(2)))});
\draw[thick] (4.95,{5 * (-0.99 * (ln(0.99) / ln(2)) - 0.01 * (ln(0.01) / ln(2)))}) -- (5,0);

% Maximum point at p=0.5, H=1
\fill (2.5,5) circle (1.5pt);
\node[above] at (2.5,5) {(0.5, 1)};
