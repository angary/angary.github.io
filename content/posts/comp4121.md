...
After $en \cdot 2 \ln n$ instances, the probability of failure of each process is less than $1/n^2$ and since there are $n$ processes, then the probability that at least one process failed is $\leq 1/n$.
...
More generally, the probability to have an element / be nonempty on level $k \log n$ is smaller than $n /2^{k \log n} = 1/n^{k - 1}$.
...
If an element has a link at level $i$ then with probability 1/2 it also has a link at level $i + 1$.
...
For an element on levels $0 - i$, we store $O(i + 1)$ pointers, and expected number of elements with highest link on level $i$ is $O(n/2^{i + 1})$. Thus, total expected space for all pointers does not exceed
...
def karger_refined(G: Graph) -> int:
    ...
    while len(V) > 2:
        for i in range(4):
            # Run random edge contraction until n / 2 vertices
            new_graph = contract(G)
            min_cut = karger_refined(new_graph)
...
For $n > 4$, $p = 1 - 1/n$ which for large values of $n$ is a very high probability, and has a final runtime of 

$$O\left(n^2 \log^3 n \right) \lt\lt O(\left(n^4\right).$$

## Randomised Hashing

If the hash function can be analysed, and a sequence of worst keys is chosen, then a lookup in a hash table can take $O(n)$.
...
Assume $x, y$ are two distinct keys. Then
...
Since there are
- $m^r$ sequences of the form $\langle a_{\textbf{1}}, ..., a_r \rangle$
...
2. For each run, pick a has function by randomly picking a vector $\vec{a} = (a_0, a_1, ..., a_r)$ such that $0 \leq a_i \leq m$ for all $i$, $o \leq i \leq r$.
3. During each run ues that function on all keys
...
Thus, the chance of a collision after $k$ hashes is smaller than $1/2$, which rapidly tends to 0. Consequently, after a few random trial-and-error attempts we will obtain a collision free hash tale of size $< 2n^2$.
...
Produce a hash table of size $M$ again, and assume that a slot $i$ of this table has $n_i$ many elements.
...
$$E\left[ \sum_{i=1}^{M}  \right] \leq \frac{n(n-1)}{n} + n = 2n - 1 < 2n.$$
...
Hence, vectors with randomly chosen coordinates have approximate the same length $\Theta(\sqrt{d})$ and any 2 such vectors are likely to be almost orthogonal.
...
- It's surface (if we have a ball of radius $r$, and another of radius $r(1 - \epsilon)$, most of the volume of the bigger ball is in the annulus outside the smaller ball).
...
1. themselves have a high rank $\rho(P_j)$
2. and do not point to an excessive number of other web pages
...
2. If $T$ is the walk length, then as $T \rightarrow \infty$, the values $N(P)/T$ should converge so the page ranks stay the same. This becomes an issue if there are disconnected graphs or if the graph is bipartite (and the probability of being in one side depends if the path length is odd or even).
...
Let $\mathbf{e}$ be 1 everywhere: $\mathbb{e}^\intercal = (1 \ 1 \ ... \ 1)$.
...
- If a webpage $P_i$ is nota  dangling webpage, he follows a link on the page $P_i$ which points to another webpage $P_j$ with prob $\frac{\alpha}{\\#(P_i)}$, or he jumps directly to a page $P_j$ with probability $\frac{1 - \alpha}{M}$.
- So if a page $P_i$ does not point to a page $P_j$ then $g_{i, j} = \frac{1 - \alpha}{M}$ (because from $P_i$ he can reach $P_j$ only by jumping randomly).
...
\begin{align*}
E(lh)
  &= 0(1 - \alpha) + 1 \alpha(1 - \alpha) + ... + k \alpha^k (1 - \alpha) + ... \\\\
  &= \alpha(1 - \alpha)(1 + 2 \alpha + ... + k \alpha^{k - 1} + ...) \\\\
  &= \frac{\alpha}{1 - \alpha}.
\end{align*}
$$

Google uses $\alpha = 0.85$, which is a value obtained empirically, giving an expected surfing length $\approx 5.7$, very close to 6 (coming from the idea of *six degrees of separation*), and it takes approx $50 - 100$ iterations for convergence.
...
- We are also given teh initial probabilities $\pi_1, \pi_2, ..., \pi_K$ of states.
...
- Now if a value $r'(i, j) > 0$ then $U_i$ liked $M_j$ above the global average
- $r'(i, j)$ can be positive or negative with equal probability
- However, some ratings may be influenced by "hype" or "trendiness" AKA  systematic biases"
- To remove this:
  - for every user $U_i$, we introduce $v_i$ standing for the "individual bias" of user $U_i$, reflecting tendency to give overall higher or lower scores.
  - for every movie $M_j$, we introduce $\mu_j$ standing for the "hype bias" of movie $M_j$ which is due to how "fashionable" the movie is (which fades with time).
...
In this way, we obtain two column vectors $\vec{u}_1$ and $\vec{v}_k$, and the similarity of the two users is measured by the cosine of the angle between these two vectors.

Recall that

$$\cos(u_i, u_k) = \frac{\langle \vec{u}_i, \vec{u}_k \rangle}{|\vec{u}_i| \cdot |\vec{u}_k|}$$

Thus, because two users have similar tastes if the two vectors point in "similar directions", then 

$$\sum(U_i, U_k) = \frac{\langle \vec{u}_i, \vec{u}_k \rangle}{|\vec{u}_i| \cdot |\vec{u}_k|}$$

We can now predict the rating a user $U_i$ would give to a movie $M_j$ they have not seen as follows:

1. Among all users who have sen $M_j$, pick $L$ many users $U_{k_l}$ with $L$ largest values of $|\sim(U_i, U_{k_l})|$ (i.e. the $L$ top similar and dissimilar values).
2. We now predict the rating user $U_i$ would give to movie $M_j$ as
   $$\pred(i, j) = \bar{r} + v_i + \mu_j + \frac{\sum_{1 \leq l \leq L} \sim(U_i, U_{k_l}) \tilde{r}(j, k_l)}{\sum_{1 \leq l \leq L} |\sim(U_i, U_{k_l})|}$$
3. We then recommend to user $U_i$ movie $M_j$ for which the predicted rating $\pred(i, j)$ is the highest 
...
- The distance between two data points $x, y \in \mathbb{R}^d$ can be defined as either
  $$d(x, y) = \sum_{i=1}^d |x_i - y_i| \quad \text{or as} \quad d(x, y) = \sqrt{\sum{i=1}^d (x_i -y_i)^2}$$
- If the scales of teh $i^{th}$ and $j^{th}$ coordinates $x_i$ and $x_j$ differ significantly, or if they are not of equal importance, we might consider instead
...
Find a partition $C = \\{C_1, ..., C_k \\}$ of a set of data points $A - \\{\mathbf{a_1}, ..., \mathbf{a_n} \\}$ into $k$ clusters, with the corresponding centers $\mathbf{c_1}, ..., \mathbf{c_k}$ which minimises the sum of distances between data points and their corresponding cluster centres.
...
This is the most frequently used center0based clustering algorithm.
...
- This penalises more for larger distances than teh k-median clustering
...
3. Replace cluster centres with teh centroids of thus obtained clusters.
4. Repeat 2 and 3 until cluster centers (and thus also clusters) stop changing.

**Claim:**

At very round $p$ of its loop, Lloyd's algorithm reduces the size of 
